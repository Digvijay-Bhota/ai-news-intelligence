...
export async function POST(request: NextRequest) {
  try {
    ...
    let body: { event_id?: number | string; event_hash?: string };
    try {
    ...
    if (!body || (body.event_id === undefined && !body.event_hash)) {
      return NextResponse.json({ error: 'event_id or event_hash required' }, { status: 400 });
    }
    ...
    const backendPath = '/api/v1/feed/read-event';
    ...
    console.error('BFF POST /api/feed/read-event Error:', message);
